defmodule SmartCityDogsWeb.AnimalStatusController do
 ...
 ##   with {:ok, %AnimalStatus{} = animal_status} <- AnimalStatuses.create_animal_status(animal_status_params) do
 ##     conn
 ##     |> put_status(:created)
 ...
 ## end
  ...
  def update(conn, %{"id" => id, "animal_status" => animal_status_params}) do
    ...
    with {:ok, %AnimalStatus{} = animal_status} <- AnimalStatuses.update_animal_status(animal_status, animal_status_params) do
      render(conn, "show.json", animal_status: animal_status)
    end
  end
