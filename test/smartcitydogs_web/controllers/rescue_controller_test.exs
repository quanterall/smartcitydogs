defmodule SmartCityDogsWeb.RescueControllerTest do
  ...
  def fixture(:rescue) do
    {:ok, rescue} = Rescues.create_rescue(@create_attrs)
    rescue
  end
  ...
  describe "update rescue" do
    ...
    test "renders rescue when data is valid", %{conn: conn, rescue: %Rescue{id: id} = rescue} do
      conn = put conn, rescue_path(conn, :update, rescue), rescue: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, rescue: rescue} do
      conn = put conn, rescue_path(conn, :update, rescue), rescue: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete rescue" do
    ...
    test "deletes chosen rescue", %{conn: conn, rescue: rescue} do
      conn = delete conn, rescue_path(conn, :delete, rescue)
      assert response(conn, 204)
      assert_error_sent 404, fn ->
        get conn, rescue_path(conn, :show, rescue)
      end
    end
  end
