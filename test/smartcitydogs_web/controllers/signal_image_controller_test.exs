defmodule SmartCityDogsWeb.SignalImageControllerTest do
  ...
  describe "update signal_image" do
    ...
    test "renders signal_image when data is valid", %{conn: conn, signal_image: %SignalImage{id: id} = signal_image} do
      conn = put conn, signal_image_path(conn, :update, signal_image), signal_image: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, signal_image: signal_image} do
      conn = put conn, signal_image_path(conn, :update, signal_image), signal_image: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
