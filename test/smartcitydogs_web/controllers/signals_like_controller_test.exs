defmodule SmartCityDogsWeb.SignalsLikeControllerTest do
  ...
  describe "update signals_like" do
    ...
    test "renders signals_like when data is valid", %{
      conn: conn,
      signals_like: %SignalsLike{id: id} = signals_like
    } do
      conn =
        put(conn, signals_like_path(conn, :update, signals_like), signals_like: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, signals_like: signals_like} do
      conn =
        put(conn, signals_like_path(conn, :update, signals_like), signals_like: @invalid_attrs)
