defmodule SmartCityDogsWeb.HeaderSlideControllerTest do
  ...
  @create_attrs %{deleted_at: ~N[2010-04-17 14:00:00.000000], image_url: "some image_url", text: "some text"}
  ...
  describe "update header_slide" do
    ...
    test "renders header_slide when data is valid", %{conn: conn, header_slide: %HeaderSlide{id: id} = header_slide} do
      conn = put conn, header_slide_path(conn, :update, header_slide), header_slide: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, header_slide: header_slide} do
      conn = put conn, header_slide_path(conn, :update, header_slide), header_slide: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
