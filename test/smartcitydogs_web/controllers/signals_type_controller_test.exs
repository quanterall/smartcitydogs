defmodule SmartCityDogsWeb.SignalsTypeControllerTest do
  ...
  describe "update signals_type" do
    ...
    test "renders signals_type when data is valid", %{conn: conn, signals_type: %SignalsType{id: id} = signals_type} do
      conn = put conn, signals_type_path(conn, :update, signals_type), signals_type: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, signals_type: signals_type} do
      conn = put conn, signals_type_path(conn, :update, signals_type), signals_type: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
