defmodule SmartCityDogsWeb.PerformedProcedureControllerTest do
  ...
  @create_attrs %{date: ~N[2010-04-17 14:00:00.000000], deleted_at: ~N[2010-04-17 14:00:00.000000]}
  ...
  describe "create performed_procedure" do
    test "renders performed_procedure when data is valid", %{conn: conn} do
      conn = post conn, performed_procedure_path(conn, :create), performed_procedure: @create_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, performed_procedure_path(conn, :create), performed_procedure: @invalid_attrs
  ...
  describe "update performed_procedure" do
    ...
    test "renders performed_procedure when data is valid", %{conn: conn, performed_procedure: %PerformedProcedure{id: id} = performed_procedure} do
      conn = put conn, performed_procedure_path(conn, :update, performed_procedure), performed_procedure: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, performed_procedure: performed_procedure} do
      conn = put conn, performed_procedure_path(conn, :update, performed_procedure), performed_procedure: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete performed_procedure" do
    ...
    test "deletes chosen performed_procedure", %{conn: conn, performed_procedure: performed_procedure} do
